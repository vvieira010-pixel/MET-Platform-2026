...
import {
  READING_PART1 as MT2_READING_PART1,
  READING_PART2 as MT2_READING_PART2,
  READING_PART3 as MT2_READING_PART3,
} from '../data/mock-test-2/reading.js';
import {
  LISTENING_PART1 as MT2_LISTENING_PART1,
  LISTENING_PART2 as MT2_LISTENING_PART2,
  LISTENING_PART3 as MT2_LISTENING_PART3,
} from '../data/mock-test-2/listening.js';
import { getPoints as getPoints2 } from '../data/mock-test-2/index.js';
...
export function getAllReadingQuestions2() {
  const qs = [];
  MT2_READING_PART1.questions.forEach(q => qs.push(q));
  MT2_READING_PART2.passages.forEach(p => p.questions.forEach(q => qs.push(q)));
  MT2_READING_PART3.textSets.forEach(ts => ts.questions.forEach(q => qs.push(q)));
  return qs;
}
...
export function scoreSection(answers, questions, pointsFn) {
  let total = 0;
  let max = 0;
  const details = [];

  questions.forEach(q => {
    const pts = (pointsFn || getPoints)(q.type, q.level);
...
export function scoreReading2(answers) {
  return scoreSection(answers, getAllReadingQuestions2(), getPoints2);
}

export function scoreListening2(answers) {
  return scoreSection(answers, getAllListeningQuestions2(), getPoints2);
}

export function scoreMockTest2(answers) {
  const reading = scoreReading2(answers);
  const listening = scoreListening2(answers);
