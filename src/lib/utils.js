...
export function lazyWithRetry(componentImport, maxRetries = 3) {
  return lazy(() => {
    let retries = 0;
      ...
      const attemptImport = async () => {
        try {
          ...
          if (retries >= maxRetries) {
            console.error('[lazyWithRetry] Max retries reached, reloading...', error);
            window.location.reload();
            return;
          }
          console.error(`[lazyWithRetry] Chunk load failed (attempt ${retries}/${maxRetries}), retrying in 5s...`, error);
