...
const SECTIONS = [
  { id: 'reading', label: 'Reading & Grammar', color: 'var(--section-reading)' },
  { id: 'listening', label: 'Listening', color: 'var(--section-listening)' },
  { id: 'speaking', label: 'Speaking', color: 'var(--section-speaking)' },
  { id: 'writing', label: 'Writing', color: 'var(--section-writing)' },
];
...
function TypeBadge({ type, level }) {
  const typeColors = {
    grammar: 'var(--type-grammar)',
    main_idea: 'var(--type-main-idea)',
    detail: 'var(--purple)',
    inference: 'var(--error)',
  };
  const levelColors = { b1: 'var(--level-b1)', b2: 'var(--level-b2)' };
  return (
            ...
            <div key={gi} className="mte-part-group">
              ...
              {group.questions.map(q => (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  onKeyDown={e => (e.key === 'Enter' || e.key === ' ') && (e.preventDefault(), handleSelect(q))}
                >
