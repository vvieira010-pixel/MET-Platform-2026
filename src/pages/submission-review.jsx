...
import {
  getFormativeRecommendation, buildSubmissionEvidence, cleanText,
  TYPE_SKILL, EMPTY_REVIEW_FORM,
} from '../lib/submission-utils.js';
      ...
      for (const err of errors) {
        ...
        const wasAlreadyCounted = previousActiveErrors.some(line => line.includes(errText) || errText.includes(line.slice(0, 20)));
          if (isCurrentMatch && !wasAlreadyCounted) {
            incrementErrorAppearance(submission.studentId, err.id).catch(err => {
              console.error('Failed to increment error appearance:', err);
              window.toast?.('Failed to update error tracking.', 'warn');
            });
          }
      }
    }
      ...
      <div className="page-shell-lg" style={{ paddingBottom: 'var(--space-12)' }}>
        <div className="grid-auto-fit-lg">
          ...
          <div className="flex-col-gap4">
            ...
            {formativeRec && (() => {
              const toneColors = {
                strong: { bg: 'var(--success-bg)', border: 'var(--success-soft)', label: 'var(--success)' },
                partial: { bg: 'var(--warning-bg)', border: 'var(--warning-soft)', label: 'var(--warning-text)' },
                weak:    { bg: 'var(--error-bg)', border: 'var(--error-soft)', label: 'var(--error)' },
              };
